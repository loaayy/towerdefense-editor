...
function ObjectInspector:onTouch(event, x, y)
    ...
    local function updateObject(changeFunction)
        if self.object_ == nil then
            --print('self.object_ == nil')
            return
        end
...
function ObjectInspector:createView(parent, object)
    ...
    closeButton:setPosition(offset, -offset + 10)
    ...
    self.positionButton_:setPosition(0, -offset + 10)
...
function ObjectInspector:setObject(object)
    ...
    if (classId == "static" or classId == "entity") then
    ...
    if object:hasBehavior("PathEditorBehavior") then
        ...
        lines[#lines + 1] = {
            ...
            value = "input number",
            edit  = true,
            editNote = "pathId is int,input 0 ~ 999",
            editFunction = function(object, newvalue)

                local pathName = string.format("path:%d",newvalue)
                local path = self.map_:getObject(pathName)
                if path ~= nil then
                    print("已经有相同的路径！！！")
                    return
                end
                self.map_:resetPathName(object:getId(),pathName)
    ...
    if object:hasBehavior("PlayerBehavior") then
         lines[#lines + 1] = {
            name  = "speed",
            value = object:getPlayerSpeed(),
            edit  = true,
            editNote = "speed is integer",
            editFunction = function(object, newvalue)

                print(object:getSpeed())
                
                object:setPlayerSpeed(checknumber(newvalue))
