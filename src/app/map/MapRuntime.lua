...
function MapRuntime:ctor(map)
    ...
    self.curObject_            = nil -- 当前选中对象
    self.touchCount_           = 0

    local eventName = MapManager:getEventName()
    local eventHandlerModuleName = string.format("maps.events.%s", eventName)
...
function MapRuntime:onTouch(event, x, y)
    if self.over_ or self.paused_  then return end

    -- 将触摸的屏幕坐标转换为地图坐标
    local mapx, mapy = self.camera_:convertToMapPosition(x, y)
    local minDist = 999999

    -- 检查是否选中了某个塔
    local selectedTowerId
    for id, tower in pairs(self.towers_) do
        local dist = math2d.dist(mapx, mapy, tower[1], tower[2])
    ...
    if event == "began" then
       ...
       for id, object in pairs(self.map_:getAllObjects()) do
            ...
            if (classId == "static" or classId == "entity") and object:checkPointIn(mapx, mapy) then
                ...
                self.curObject_ = object

                return self.curObject_:isSelected()
            else
    ...
    elseif event == "moved" then
        ...
        --print(self.touchCount_)

        if self.curObject_ and self.curObject_:hasBehavior("PlayerBehavior") then
            
            local nx, ny = x + self.drag_.offsetX, y + self.drag_.offsetY

            if self.curObject_:isSelected() then
                self.curObject_:setPosition(nx,ny)
                self.curObject_:updateView()
            end
    ...
    elseif event == "ended" then
       ...
       for id, object in pairs(self.map_:getAllObjects()) do
            ...
            local classId = object:getClassId()
            if (classId == "static" or classId == "entity") and 
            object:hasBehavior("PlayerBehavior") then

                if self.touchCount_ > 0 then
                    self.touchCount_ = 0
                    --print(self.touchCount_)
                else
                    object:stopMoving()
                    object:goTo(mapx,mapy)
